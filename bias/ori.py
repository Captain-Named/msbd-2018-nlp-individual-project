...
def get_span(seq1, seq2):
    ...
    seq1 = [str(x) for x in seq1.tolist()]  # seq可能是nparr，先转list再把元素转str
    seq2 = [str(x) for x in seq2.tolist()]

    matcher = difflib.SequenceMatcher(None, seq1, seq2) # None无预处理操作
    template1, template2 = [], []
    for op in matcher.get_opcodes():
        ...
        if op[0] == 'equal':
            template1 += [x for x in range(op[1], op[2], 1)]    # template1: 维护pro_idx_start~end 当前op 对序列1的操作范围
            template2 += [x for x in range(op[3], op[4], 1)]    # anti_idx_start~end 当前op 对序列2的操作范围

    return template1, template2


def mask_unigram(data, lm, n=1):    # data是一条数据
    """
    ...
    for i in range(1, N-1):
        ...
        sent1_masked_token_ids[0][template1[i]] = mask_id   # sent1_masked_token_ids[0] / [0]是在做什么？
...
def evaluate(args):
    ...
    log_softmax = torch.nn.LogSoftmax(dim=0)    # log-softmax层 把模型输出先取softmax变成概率，再取log变成log概率
    ...
    with tqdm(total=total) as pbar: # 显示循环进度
